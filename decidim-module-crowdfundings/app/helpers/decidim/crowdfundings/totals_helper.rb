...
module Decidim
  module Crowdfundings
    ...
    module TotalsHelper
      ...
      def totals_title(user)
        if user.nil?
          return I18n.t("overall_totals",
                        scope: "decidim.crowdfundings.campaigns.totals")
        end
      ...
      # block
      def total_collected(campaign, user)
        return total_collected_to_currency campaign.total_collected if user.nil?

        total_collected_to_currency campaign.user_total_collected(user)
      end

      # PUBLIC: Returns percentage value formatted as a percentage without
      # decimal places.
      def percentage(campaign, user)
        value = percentage_value(campaign, user)
      ...
      def percentage_value(campaign, user)
        return campaign.percentage if user.nil?
        campaign.user_percentage(user)
      end
      ...
      def percentage_tag(campaign, user)
        css_class = percentage_class(percentage_value(campaign, user))
