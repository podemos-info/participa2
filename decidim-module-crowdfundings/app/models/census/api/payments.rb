  ...
  module API
    ...
    class Payments
      ...
      def create_order(qualified_id, **params)
        params[:amount] *= 100
        process_response(
          send_request { post("/api/v1/payments/orders", params.merge(person_id: qualified_id)) }
        )
      end
      ...
      def payment_methods(qualified_id)
        process_response(
          send_request { get("/api/v1/payments/payment_methods", person_id: qualified_id) }
      ...
      def payment_method(payment_method_id)
        ...
        process_response(
          send_request { get("/api/v1/payments/payment_methods/#{payment_method_id}") }
      ...
      def orders_total(**params)
        process_response(
          send_request { get("/api/v1/payments/orders/total", params.slice(:person_id, :campaign_code, :from_date, :until_date)) }
        ) do |response|
          response[:amount] / 100
        end
      end
